...
( function ( $, window, document, undefined ) {
	var	PREFIX =  "-" + getVendorPrefix().toLowerCase() + "-",
		...
		methods = {
			...
			getPath: function( options ) {
				...
				return pathObject || ( pathObject = new jPath( speeds.scrollSpeed, speeds.rotationSpeed ));
			},
	...
	function jPath( scrollS, rotateS ) {
		var PADDING = 40,
			scrollSpeed = scrollS,
		...
		this.rotate = function( radians, options ) {
			...
			if( settings.name ) nameMap[ settings.name ] = jpath.length - 1;
		...
		this.moveTo = function( x, y, options ) {
			var settings = $.extend( {}, defaults, options ),
				steps = jpath.length ? STEP_SIZE : 1;
				i = 0;
		...
		this.arc = function( centerX, centerY, radius, startAngle, endAngle, counterclockwise, options ) {
			...
			for ( ; i <= steps; i++ ) {
				jpath.push({ x: centerX + radius * Math.cos( startAngle + radStep*i ),
	...
	function initCanvas() {
		...
		var jcanvas,
			style = {
				position: "absolute",
				"z-index": 500,	//ratz :: change as required
		...
		jcanvas[ 0 ].width = pathObject.getPathWidth();
		jcanvas[ 0 ].height = pathObject.getPathHeight();
		
		drawCanvasPath( jcanvas[ 0 ].getContext( "2d" ), pathObject.getCanvasPath() );
	}

	/* Sets the canvas path styles and draws the path */
	function drawCanvasPath( context, jpath ) {
		var i = 0;

		context.shadowBlur = 15;			//ratz :: change as required
		...
		for( ; i < jpath.length; i++ ) {
			context[ jpath[ i ].method ].apply( context, jpath[ i ].args );
		}
	...
	function scrollHandler( e ) {
		...
		var spvSpeedFactor = $("#spvSpeedFactor").attr("value");
		scrollSteps( -dir * STEP_SIZE * spvSpeedFactor);
	}
	...
	function scrollToStep( stepParam, fromAnimation ) {
		...
		if($("#spvDetachScroll").attr("value")==1) return; //ratz :: block the scrolling
		var cb;
		...
		//ratz :: scroll percentage
		 	var currPerc = (stepParam / (pathList.length - 1 ) * 100);
			//$("#spvCurrPerc").attr("value",currPerc);
		if ( cb && stepParam !== step && !isAnimating ) cb();
		step = stepParam;
		//ratz :: output motion sense so that paper route can be easily drawn
			var ty = $(".mapContainer").css("top");
			var tyN = Math.round((-ty.substr(0,ty.length-2)));
			var tx = $(".mapContainer").css("left");
			var txN = Math.round((-tx.substr(0,tx.length-2)));		
			var fx = txN + window.innerWidth/2;
			var fy = tyN + window.innerHeight/2;
			addThisPaperPoint(fx,fy,currPerc);
	}
	...
	//ratz :: global vars to calc transport angle & pos
		var lastNodeX = 0, lastNodeY = 0
	...
	function makeCSS( node ) {
		...
		//ratz :: transport angle calc
			xDelta = node.x - lastNodeX;
			yDelta = node.y - lastNodeY;
			slope = yDelta/xDelta;
			theta = Math.atan(slope);	//angle with respect to the node, have to add node.rotate value to get global angle
			thetaDeg = theta * 180 / Math.PI;	
			//if ( thetaDeg < 0 ) thetaDeg = 180 - thetaDeg;
			nodeRotDeg = node.rotate * 180 / Math.PI;	
						//node rotate angle - the rotation of the map wrto the windows
			thetaTot = thetaDeg + nodeRotDeg;
			if ( xDelta < 0 ) {
				thetaTot2 = 180 + thetaTot; //arcTan domain-range correction for actual values 
				$(".transport").css("webkit-transform", "rotate(" + thetaTot2 + "deg)");
				$(".transport").css("moz-transform", "rotate(" + thetaTot2 + "deg)");
				$(".transport").css("o-transform", "rotate(" + thetaTot2 + "deg)");
				$(".transport").css("ms-transform", "rotate(" + thetaTot2 + "deg)");
				$(".transport").css("transform", "rotate(" + thetaTot2 + "deg)");
			}
			else
			{ 
				$(".transport").css("webkit-transform", "rotate(" + thetaTot + "deg)");
				$(".transport").css("moz-transform", "rotate(" + thetaTot + "deg)");
				$(".transport").css("o-transform", "rotate(" + thetaTot + "deg)");
				$(".transport").css("ms-transform", "rotate(" + thetaTot + "deg)");
				$(".transport").css("transform", "rotate(" + thetaTot + "deg)");
			}
			lastNodeX = node.x; 
			lastNodeY = node.y;
